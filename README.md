...
129のMCPツール + Knowledge Store（暗黙知の記憶） + Claude Skill（行動規範）の3層構造で、単なるAPIラッパーではなく**あなたの会社のHubSpotを理解したMA担当者**として機能します。
...
MCPサーバーを接続するだけ。HubSpotの129 APIをAIから直接操作できます。
...
> **Skillなしでも使えます。** MCPサーバーだけ接続すれば130ツールのAPIラッパーとして機能します。Skillを追加すると「MA担当者」としての判断力が加わります。

### スキーマ品質ポリシー

各 MCP ツールの入力 Zod スキーマは以下の方針で構築されています：

- **重要ツール（form_create / form_update など）は実物API構造で完全型化**：実機の `*_get` レスポンス構造をベースに、HubSpot Marketing Forms v3 / Automation v4 / CMS API が要求する全フィールド（`validation`、`options`、`defaultValues`、`useCountryCodeSelect`、`notifyRecipients`、`recaptchaEnabled`、`embedType` 等）を Zod で網羅。
- **全 object スキーマに `.passthrough()`**：HubSpot API の将来のフィールド追加や、ここで明示していない隠しフィールドを素通しできるよう、全ての object スキーマで未知のキーを許可。
- **隠れ必須フィールドの自動補完**：`form_create` / `form_update` は HubSpot API が要求する `createdAt` / `updatedAt` をサーバ側で自動補完。ユーザーが毎回タイムスタンプを指定する必要なし。

> **ツールを追加・修正するときは必ず [docs/development-checklist.md](./docs/development-checklist.md) を参照してください。**
> 公式OpenAPI specの取得方法、required/enumの完全網羅、テスト手順をまとめてあります。
> 過去の事故事例（form_create configuration 不足、postSubmitAction.type の `redirect` 誤り、theme の `none` 誤り、workflow batch_read の type 不足）の振り返りも含みます。
...
> Skillなしでも130ツールはそのまま使えます。Skillを入れると「MA担当者」としての判断力（Knowledge Store活用・自己検証ループ・施策パターン）が加わります。
...
| `form_patch` | **PATCH（部分更新）。** 指定したフィールドだけが上書きされ、省略した部分は既存値を保持。`configuration.notifyRecipients` の追加など部分更新に最適。 |
...
| CMS Blog & Pages | v3 | 5 | list / update / **replace_form_widget**（form_id差し替え専用） |
| Owners | v3 | 2 | list / get |
| Forms | Marketing Forms v3 | 6 | list / get / create / update / **patch**（部分更新） / delete |
...
**Q: Skillなしでも使える？**
→ はい。MCPサーバーだけで130ツールのAPIラッパーとして機能します。Skillを追加するとMA担当者としての判断力が加わります。
...
同一の `lib/`（130ツール + HubSpotクライアント）を Cloudflare Workers 上で実行する構成。I/O待ち時間が課金されないため、外部API呼び出しが多い用途ではコスト効率が高い。
...
| **hubspot-ma-mcp** ← 今ここ | 129 | HubSpot MA（CRM/Marketing/Knowledge Store） |
