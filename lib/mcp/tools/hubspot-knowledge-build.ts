...
自力で答えが出る質問を解決してから、残った質問だけをユーザーに提示すること。`,
    ...
    async ({ force }) => {
      try {
        ...
        // --- goals ---
        const goLines: string[] = ["## マーケティングKPI目標（要入力）\n"];
        goLines.push("### 四半期目標\n- （未入力: 例: リード獲得200件、メール開封率30%、セミナー月1回等）\n");
        goLines.push("### 月次目標\n- （未入力）\n");
        goLines.push("### 達成基準\n- （未入力: 何をもって目標達成とするかの定義）\n");
        questions.push("[goals] マーケティングのKPI目標は現在設定されていますか？（例: 四半期でリード200件獲得等）なければNoで構いません。目標がなくても施策実行は可能です。");
        drafts.goals = goLines.join("\n");

        // --- calendar ---
        const calLines: string[] = ["## 施策カレンダー（要入力）\n"];
        calLines.push("### 今月の予定\n- （未入力）\n");
        calLines.push("### 来月の予定\n- （未入力）\n");
        calLines.push("### 定期施策\n- （未入力: 例: 毎月第1月曜にNL配信、四半期ごとにセミナー等）\n");
        questions.push("[calendar] 定期的な施策スケジュールはありますか？（例: 毎月NL配信、四半期セミナー等）なければNoで構いません。");
        drafts.calendar = calLines.join("\n");
