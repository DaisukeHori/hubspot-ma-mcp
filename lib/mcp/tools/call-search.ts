...
export function registerCallSearch(server: McpServer) {
  server.tool(
  "call_search",
  "HubSpotの通話エンゲージメントを検索する。返却値: total, results（hs_createdate, hs_lastmodifieddate, hs_object_id）, paging。制約: 最大5 filterGroups×各6 filters（合計18 filters）、総結果上限10,000件。",
  {
    ...
    properties: z.array(z.string()).optional().describe("取得するプロパティ名の配列（例: ['hs_call_title']）。省略時はデフォルトプロパティのみ"),
  ...
  async ({ query, filterGroups, properties, limit, after, sorts }) => {
    const result = await crmSearch("calls", query || "", properties, filterGroups, limit || 10, after, sorts);
    return {
