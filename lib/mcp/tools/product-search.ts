...
返却: 一致する商品の配列（ID, プロパティ, 作成日, 更新日）。totalで総件数も返る。制約: 最大5 filterGroups×各6 filters（合計18 filters）、総結果上限10,000件。
ページネーション: afterに前回レスポンスのカーソルを指定して次ページ取得。`,
    {
      query: z.string().optional().describe("フリーテキスト検索キーワード。name, description, price, hs_skuを横断検索。filterGroupsと併用可能"),
    ...
    async ({ query, filterGroups, properties, limit, after, sorts }) => {
      const result = await crmSearch("products", query || "", properties || DEFAULT_PROPS, filterGroups, limit || 10, after, sorts);
