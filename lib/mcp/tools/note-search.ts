...
返却: 一致するメモの配列（ID, hs_note_body, hs_timestamp等）。totalで総件数も返る。制約: 最大5 filterGroups×各6 filters（合計18 filters）、総結果上限10,000件。
ページネーション対応。`,
    {
      query: z.string().optional().describe("フリーテキスト検索キーワード。hs_note_bodyを横断検索。filterGroupsと併用可能。注意: hs_body_preview_htmlはフィルタ不可"),
      filterGroups: z
        .array(
          z.object({
            filters: z.array(
